...
async function fetchAllRows(buildQuery, label) {
  ...
  for (let offset = 0; ; offset += SUPABASE_PAGE_SIZE) {
    ...
    if (error) {
      console.error(`[Users] ${label} error:`, error);
      return rows;
    }
    ...
    if (rows.length >= SUPABASE_MAX_ROWS) {
      console.warn(`[Users] ${label} stopped at safety limit ${SUPABASE_MAX_ROWS}`);
      break;
    }
  }
...
// 記錄推播點擊
export async function recordPushClick(userId, linkId, options = {}) {
  const clickedAt = options.clickedAt || new Date().toISOString();
  const { data: user } = await supabase
    .from('official_line_users')
  ...
  // 記錄點擊明細
  const { error: clickError } = await supabase.from('official_line_clicks').insert({
    line_user_id: userId,
    link_id: linkId,
    clicked_at: clickedAt,
  });
  if (clickError) console.error('[Users] click insert error:', clickError);

  if (/^drip_\d+$/.test(String(linkId || ''))) {
    const { error: dripError } = await supabase
      .from('official_drip_logs')
      .update({ clicked: true, clicked_at: clickedAt })
      .eq('line_user_id', userId)
      .eq('link_id', linkId);
    if (dripError) console.error('[Users] drip click update error:', dripError);
  }
}
...
export async function getUserPathState(userId) {
  const { data } = await supabase
    ...
    .select(
      ...
      // V3.2 Phase 2B Session 3：補 q5_msg{1-4}_sent_at，dispatcher priority 1（Ch.5.10）
      // 跟 handleV32FreeText 算當前段 N（Ch.6.3）都靠這 4 欄。
      'path, path_stage, current_weight, target_weight, ai_tags, ai_tags_updated_at, last_user_reply_at, last_stage5_reply_at, path_stage_updated_at, handoff_triggered_at, handoff_reason, handoff_rescue_notified, is_blocked, metabolism_type, q5_sent_at, q5_intent, q5_msg1_sent_at, q5_msg2_sent_at, q5_msg3_sent_at, q5_msg4_sent_at'
    )
    .eq('line_user_id', userId)
    .single();
  return data || {};
