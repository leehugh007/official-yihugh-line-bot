...
const ALLOWED_KEYS = new Set([
  ...
  // 2026-04-24：Q2 自由文字「其他狀況」當 path_choice 選項外的出路
  // 用戶打「我狀況很複雜…」這種自由文字 → 寫進 ai_tags 供婉馨看 + triggerHandoff
  'q2_free_text_note', // 用戶在 Q2 path_choice 講的自由文字原文（供 fifi 看）
  'q2_free_text_at', // ISO timestamp，寫入時間
]);
...
export async function getUserPathState(userId) {
  const { data } = await supabase
    .from('official_line_users')
    .select(
      'path, path_stage, current_weight, target_weight, ai_tags, ai_tags_updated_at, last_user_reply_at, last_stage5_reply_at, path_stage_updated_at, handoff_triggered_at, handoff_reason, handoff_rescue_notified, is_blocked, metabolism_type'
    )
    .eq('line_user_id', userId)
    .single();
  return data || {};
