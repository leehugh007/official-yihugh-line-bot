...
export async function getUserPathState(userId) {
  const { data } = await supabase
    .from('official_line_users')
    .select(
      'path, path_stage, current_weight, target_weight, ai_tags, ai_tags_updated_at, last_user_reply_at, last_stage5_reply_at, path_stage_updated_at, handoff_triggered_at, handoff_reason, handoff_rescue_notified, is_blocked'
    )
    .eq('line_user_id', userId)
    .single();
  return data || {};
