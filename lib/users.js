  ...
  const { error } = await supabase.from('official_line_users').upsert(
    {
      ...
      source: data.source || 'direct', // 'quiz' | 'direct' | 'seminar'
      joined_at: now,
      last_interaction_at: now,
      interaction_count: 0,
      push_click_count: 0,
      segment: 'new', // 'new' | 'active' | 'warm' | 'silent'
      is_blocked: false,
      ...data,
    },
...
export async function recordPushClick(userId, linkId) {
  ...
  const isSignupLink = SIGNUP_KEYWORDS.some((kw) => linkId.toLowerCase().includes(kw));
  if (isSignupLink) {
