...
import { getPricingState } from './pricing.js';
...
const MSG2_TEXT = `如果你以前瘦過，後來又胖回來，
我不會先叫你更自律。
...
A — 加營養（Add）：把身體缺的補回來
B — 調體質（Behavior）：把讓你失控的日常行為調順
C — 輕負擔（Clear）：把身體跟心理的負擔清掉

我帶過 3,000 多個學員後最確定的一件事是：
順序對了，人才不用一直靠意志力。

想看看這 12 週我們怎麼陪你把這件事做起來嗎？`;

// MSG3 fallback：當 pricing setting 缺漏時退回此版本（避免訊息推不出去）
const MSG3_TEXT_FALLBACK = `如果你看到這裡，下一步不是要你立刻決定一輩子的事。

下一步只是讓 fifi 看你的狀況，
確認你適不適合這一班，也確認你比較適合 12 週還是 4 週。
...
你不用因為早鳥衝動，但如果你心裡知道自己不想再用同一套方法重來，
可以先讓我們幫你看一次。`;

// 台北時區日期 helper（給 MSG3 三段價日期文案用）
function _formatTpeDate(isoStr) {
  if (!isoStr) return '';
  const d = new Date(isoStr);
  if (isNaN(d.getTime())) return '';
  // en-CA 給 YYYY-MM-DD，符合需求
  return d.toLocaleDateString('en-CA', { timeZone: 'Asia/Taipei' }).replace(/-/g, '/');
}

function _formatTpeMonthDay(isoStr) {
  if (!isoStr) return '';
  const d = new Date(isoStr);
  if (isNaN(d.getTime())) return '';
  return d.toLocaleDateString('en-CA', { timeZone: 'Asia/Taipei' }).slice(5).replace('-', '/');
}

function _addOneDayIso(isoStr) {
...
/**
 * 產出 MSG3 文案（三段價依後台 setting 顯示具體日期）
 * setting 缺漏時 fallback 回 MSG3_TEXT_FALLBACK，避免訊息推不出去
 */
function renderQ5Template(template, replacements) {
...
function formatProgramStartDate(pricing) {
  const raw = pricing?.batch?.start_date;
  if (!raw) return '開課日期以官方公告為準';
  const normalized = /^\d{4}-\d{2}-\d{2}$/.test(raw) ? `${raw}T00:00:00+08:00` : raw;
  const formatted = _formatTpeDate(normalized);
  return formatted ? `${formatted} 開學` : '開課日期以官方公告為準';
}

export function buildMsg3Text(pricing) {
  const superCutoff = pricing?.super_cutoff_at;
  const regularCutoff = pricing?.regular_cutoff_at;
  const batchName = pricing?.batch?.name || '下一期班';
  const startDate = formatProgramStartDate(pricing);
  if (!superCutoff || !regularCutoff) {
    return renderQ5Template(pricing?.q5_templates?.msg3 || MSG3_TEXT_FALLBACK, {
      batch_name: batchName,
      start_date: startDate,
      price_lines: '‧ 早鳥優惠價（詳見報名頁）\n‧ 早鳥結束後恢復原價',
    });
  }

  const superPrice = pricing?.prices?.super ?? 10400;
  const regularPrice = pricing?.prices?.regular ?? 11400;
  const anchorPrice = pricing?.prices?.anchor ?? 12600;

  const superDateStr = _formatTpeDate(superCutoff);                          // 2026/05/18
  const regularStartStr = _formatTpeMonthDay(_addOneDayIso(superCutoff));    // 05/19
  const regularEndStr = _formatTpeMonthDay(regularCutoff);                   // 05/24
  const anchorStartStr = _formatTpeDate(_addOneDayIso(regularCutoff));       // 2026/05/25

  // 2026-08-22：超早鳥已取消（8/12 決定），但 setting 仍留著 7/20 的舊截止 → 只有「還沒截止」才渲染超早鳥行，
  // 否則兩段（早鳥優惠 / 原價），跟報名頁 & pricing.js 的 tier 判斷一致。事故：Miki 收到「超早鳥 10,400 > 早鳥 9,900」倒掛
  const superActive = new Date(superCutoff).getTime() > Date.now();
  const regularEndFull = _formatTpeDate(regularCutoff);                    // 2026/09/01
  const priceLines = superActive
    ? `‧ ${superDateStr} 前 — 超早鳥 $${superPrice.toLocaleString()}
‧ ${regularStartStr}–${regularEndStr} — 早鳥優惠 $${regularPrice.toLocaleString()}
‧ ${anchorStartStr} 之後 — 原價 $${anchorPrice.toLocaleString()}`
    : `‧ ${regularEndFull} 前 — 早鳥優惠 $${regularPrice.toLocaleString()}
‧ ${anchorStartStr} 之後 — 原價 $${anchorPrice.toLocaleString()}`;
  const defaultTemplate = `如果你看到這裡，下一步不是要你立刻決定一輩子的事。
...
你不用因為早鳥衝動，但如果你心裡知道自己不想再用同一套方法重來，
可以先讓我們幫你看一次。`;
  return renderQ5Template(pricing?.q5_templates?.msg3 || defaultTemplate, {
    batch_name: batchName,
    start_date: startDate,
...
早鳥截止是真的，{{batch_name}}開課也是真的。
錯過不是不能再來，只是這一班的陪伴和價格就過去了。

如果你還在想，可以先讓 fifi 看你適不適合。`;

function buildMsg4Text(pricing) {
  return renderQ5Template(pricing?.q5_templates?.msg4 || MSG4_TEXT, {
    batch_name: pricing?.batch?.name || '下一期班',
    start_date: formatProgramStartDate(pricing),
  });
}
...
export async function buildQuickReplyForStage(n, { userId, triggerSource = 'active' } = {}) {
  if (n === 1) {
    return _qr([
      _postbackAction('我想看 ABC 怎麼幫我', 'action=intro_next'),
  ...
  if (n === 2) {
    return _qr([
      _postbackAction('想看 12 週怎麼陪我做', 'action=method_next'),
      _postbackAction('我想先確認適不適合', 'action=method_question'),
  ...
  if (n === 3 || n === 4) {
    ...
    return _qr([
      _uriAction('先讓 fifi 看我適不適合', url),
      _postbackAction('我想問最後一個問題', 'action=final_question'),
...
export async function pushMsg3(userId, replyToken, triggerSource = 'active') {
  ...
  // 三段價動態文案：讀後台 setting 顯示具體日期（fallback 回原文案）
  let msg3Text;
  try {
    const pricing = await getPricingState();
    msg3Text = buildMsg3Text(pricing);
  } catch (err) {
    console.error('[Q5MsgState] pushMsg3 pricing read failed, using fallback:', err.message);
    msg3Text = buildMsg3Text({});
  }

  const msg = { type: 'text', text: msg3Text, quickReply };
  if (replyToken) {
    await replyMessage(replyToken, [msg]);
  } else {
    await pushMessage(userId, [msg]);
...
export async function pushMsg4(userId, triggerSource = 'passive') {
  ...
  if (!claimed) return false;
  let msg4Text = MSG4_TEXT;
  try {
    msg4Text = buildMsg4Text(await getPricingState());
  } catch (err) {
    console.error('[Q5MsgState] pushMsg4 pricing read failed, using fallback:', err.message);
    msg4Text = buildMsg4Text({});
  }
  await pushMessage(userId, [{ type: 'text', text: msg4Text, quickReply }]);
  return true;
}
...
/**
 * 標訊息 N 的用戶回應（不寫 sent_at）+ race guard
 *
 * schema 限制：訊息 3/4 沒有 replied_at（決策見 schema 對齊段，msg3/4 後直接 URI 不算 replied）
 *
 * race guard（Ch.5.2/5.3/5.4 「conditional UPDATE，已寫 → race lost return false」）：
 *   conditional UPDATE q5_msg{n}_{suffix} = NOW() WHERE IS NULL
 *   無 row 影響 = race lost → return false → 呼叫方應 reply「已收到」避免重複處理
 *
 * @param {string} userId
 * @param {1|2|3|4} n
...
export const NEGATION_RE = /不[想了解OK好]|沒[興趣意願]/;
