// 契約 v2.4 Ch.8：settings 常數（DB fallback）
// 型別 = JS native（不是字串）。DB 值格式見契約 Ch.8.1。

export default {
  weight_diff_small_max: 5,
  weight_diff_large_min: 15,
  fallback_threshold: 2,
  stage_timeout_days: 7,
  reawaken_days: 14,
  ai_tags_expire_days: 14,
  handoff_notify_to: ['yixiu', 'wanxin'],
  handoff_rescue_notify_to: ['yixiu'],
  handoff_rescue_hours: 48,
  meal_min_chars: 20,
  postpartum_min_chars: 5,
  multi_condition_max: 2,
  min_msg_chars_for_ai: 3,
  ai_call_timeout_ms: 10000,
  handoff_keywords_price: ['價格', '費用', '多少錢', '開課', '下一期', '學費', '方案'],
  handoff_keywords_family: ['老公', '先生', '家人', '老婆', '一起', '女友', '男友', '媽媽'],
  handoff_keywords_enroll: ['報名', '我要試', '直接買', '我加入'],
  ai_polite_end_keywords: ['太貴', '沒預算', '沒錢', '先不用'],
  webhook_template_cache_ttl_sec: 60,
  gemini_model_version: 'gemini-2.5-flash-lite',
  contract_version: 'v2.4',
  test_mode: true,
  // Q5 轉換漏斗（Phase 4.1 新增）
  q5_intent_min_text_chars: 2,
  q5_active_followup_hours: 24,
  q5_visit_followup_hours: 24,
  q5_visit_followup_max_count: 1,
  q5_timeout_reset_hours: 48,
  plan_12weeks_price: 11400,
  plan_4weeks_price: 4980,
  apply_refund_policy_text: '待定',
  q5_test_mode_cron: true,
  // Phase 4.2 PR #52：Q5 限測試用戶（/apply 未完整 + 報名流程未定前的 scope gate）
  // true  = 只有 TEST_ALLOWLIST（一休+婉馨）走 Q5；其他人走 Phase 3.3 bridging / PR #48 triggerHandoff
  // false = 全量開放（/apply 完整 + 報名流程定案後切）
  q5_restricted_to_test_users: true,
  q5_soft_invite_passive_text:
    '剛剛那些問題我都有看到。下一步不是要你立刻報名，是先讓 fifi 看你適不適合這一班。要先讓她幫你確認嗎？',
  q5_soft_invite_active_text:
    '之前跟你聊到的那些卡關，我有一套帶學員時在用的做法。你不用急著決定，可以先讓 fifi 看你適不適合。要先確認嗎？',
  q5_non_text_soft_handoff_text:
    '我有看到你的問題。這個我請 fifi 直接跟你聊，她看過你剛剛跟我聊的內容，會知道你在哪個階段，等等會主動找你。\n\n先不急著決定要不要進課程，把問題問清楚再說。',
  // Phase 4.2 Ch.5.4：visit-followup cron push 文案
  // 用戶點了 /apply 沒填表單 24h 後推一次（契約 Ch.5.4）
  q5_visit_followup_text:
    '我看到你剛剛有進去看介紹頁。\n不用急著報名，我只是想確認你是不是卡在某個點。\n\n你比較想先確認哪一個？\n1. 價格 / 付款方式\n2. 我適不適合\n3. 時間 / 課程怎麼上\n4. 要跟家人討論\n5. 想直接問 fifi\n\n直接回數字或文字都可以。',
  phase_4_2_launch_date: '',
  apply_url_base: 'https://official-yihugh-line-bot.vercel.app/apply',
  program_batch_name: '下一期班',
  program_start_date: '',
  q5_msg3_template: '',
  q5_msg4_template: '',
  // /apply 方案說明（未編輯前=原文案，行為零差異）
  apply_plan12_desc: '這是多數學員選的版本。代謝重建需要時間 —— 四週打基礎，八週讓身體習慣，十二週讓它變成你的生活。',
  apply_plan12_bullets: '一休親自直播 12 堂 + 錄播課 24 堂\n15 位國家高考營養師，每天幫你看餐\n24 堂營養課 + 150+ 堂運動課\n班級制（教練、助教、班長全程陪伴）',
  apply_plan4_desc: '適合「想先看看方向對不對再決定」的人。\n一休直播課 + 營養師看餐 + 運動課，體驗一個月。',
  apply_plan_duo_desc: '內容跟 12 週完整版一樣，每人都有自己的直播 / 營養師看餐 / 運動課 / 班級。',
  // 3天看餐體驗自動邀請（lib/trial-invite.js）
  // enabled 預設 false = 部署後 cron 空轉；restricted 預設 true = 只發一休+婉馨測試
  trial_invite_enabled: false,
  trial_invite_restricted: true,
  trial_invite_daily_cap: 30,
  // follow 直加即邀（2026-08-22）：false = 直接加官方 OA 的人不自動收體驗邀請（kill switch，不用重新部署）
  trial_invite_on_follow: true,
};
