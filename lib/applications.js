...
import { syncApplicationToSheet, isSheetSyncEnabled } from './google-sheets.js';
 ...
 * @param {boolean} opts.mask  default true — 匯出 CSV 時設 false 拿完整 payment_last5
 * @returns {Promise<{rows: Array, total: number}>}
 */
export async function listApplications({ filter = 'all', limit = 50, offset = 0, mask = true } = {}) {
  let query = supabase
  ...
  return {
    rows: mask ? (data || []).map(maskRow) : (data || []),
    total: count || 0,
  };
}
...
export async function markApplicationPaid(id, { last5, amount, date, marked_by }) {
  ...
  // 標 paid 成功後：自動加 tag「已報名減重班」（GoogleSheet 整合方案 Y）
  // 規則：tag 只加不減，重複加無害（Set 會去重）
  if (data.line_user_id) {
    await _ensureEnrolledTag(data.line_user_id);
  }

  // 同步到 GoogleSheet（環境變數沒設時靜默跳過，失敗不擋主流程）
  if (isSheetSyncEnabled()) {
    try {
      await syncApplicationToSheet(id, data);
    } catch (err) {
      console.error('[applications/mark_paid] sheet sync silent fail:', err?.message);
    }
  }
...
/**
 * 加「已報名減重班」tag（只加不減）
 * 設計：標 paid = 確認報名完成，順手把用戶分群標好，省 Annie 重複點「標記已報名」
 */
async function _ensureEnrolledTag(lineUserId) {
  try {
    const { data: user } = await supabase
      ...
      .maybeSingle();
    const existing = Array.isArray(user?.tags) ? user.tags : [];
    if (existing.includes('已報名減重班')) return;
    const next = [...new Set([...existing, '已報名減重班'])];
    const { error } = await supabase
      .from('official_line_users')
      .update({ tags: next })
      .eq('line_user_id', lineUserId);
    if (error) {
      console.error('[applications/ensureEnrolledTag] update failed:', lineUserId, error.message);
    }
  } catch (err) {
    console.error('[applications/ensureEnrolledTag] silent fail:', err?.message);
  }
}
...
export async function markApplicationCancelled(id, { notes, marked_by }) {
  ...
  // 同步到 GoogleSheet（取消不動「已報名減重班」tag — tag 只加不減）
  if (isSheetSyncEnabled()) {
    try {
      await syncApplicationToSheet(id, data);
    } catch (err) {
      console.error('[applications/cancel] sheet sync silent fail:', err?.message);
    }
  }
...
export async function updatePaymentInfo(id, { last5, amount, date, notes, marked_by }) {
  ...
  // 同步到 GoogleSheet（含 notes / payment_* 編輯後的最新值）
  if (isSheetSyncEnabled()) {
    try {
      await syncApplicationToSheet(id, data);
    } catch (err) {
      console.error('[applications/update_payment] sheet sync silent fail:', err?.message);
    }
  }
