...
function prepareFlexButton(button, linkId, index) {
  if (button.actionType === 'message' || (!button.url && (button.replyText || button.messageText))) {
    return {
      ...button,
      actionType: 'message',
  ...
  return {
    ...button,
    url: wrapLink(button.url, `${linkId}_b${index}`),
  };
}
...
export async function sendScheduledPush(logId) {
  ...
  if (useFlexMsg) {
    const cleanButtons = (log.buttons || []).filter(isUsableFlexButton);
    const trackedButtons = cleanButtons.map((btn, i) => prepareFlexButton(btn, log.link_id, i));
