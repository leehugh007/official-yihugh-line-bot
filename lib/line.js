...
// 回覆訊息（用於 webhook 回覆）
// 回傳 res.ok（true = 成功，false = LINE API 失敗），跟 pushMessage 行為一致
// 修法時間：2026-05-04 — 之前漏 return 害 replyStoryFlex 永遠回 ok=false → handleQ4Continue 誤 fall through performQ5Transition → path_stage 被升 6 → pushMsg3 lte=4 guard 失敗 → 訊息 3 黑洞
...
export async function pushMessage(userId, messages) {
  const result = await pushMessageWithResult(userId, messages);
  return result.ok;
}

export async function pushMessageWithResult(userId, messages) {
  const body = {
    to: userId,
    messages: Array.isArray(messages) ? messages : [messages],
  };
  ...
  return { ok: res.ok, status: res.status, errorText };
}
...
function flexButtonAction(btn) {
  const isMessageButton = btn.actionType === 'message' || (!btn.url && (btn.replyText || btn.messageText));
  if (isMessageButton) {
    return {
      type: 'message',
      label: btn.label,
      text: btn.messageText || btn.label,
    };
  }

  return { type: 'uri', label: btn.label, uri: btn.url };
}

// 推播用 Flex Message（支援 1-2 個按鈕 + hero image）
// buttons = [{ label, url }, ...] 或 message action；url 請先用 wrapLink 包好
...
export function pushFlexMessage({ title, body, buttons, imageUrl }) {
  ...
  // hero image（選填）
  if (imageUrl) {
    const normalizedImageUrl = normalizePublicUrl(imageUrl);
    bubble.hero = {
      type: 'image',
