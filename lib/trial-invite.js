...
export const TAG_INVITED_DIRECT = '體驗邀請-direct'; // 直接加官方 OA（非測驗來）follow 即邀（2026-08-22 一休拍板，配合 FB 私訊新門）
...
export function buildInviteN(user, { withGreeting = true } = {}) {
  // withGreeting=false：follow 路徑歡迎訊息已自我介紹過，邀請用「對了——」接續，避免連續兩次「我是一休」
  const opening = withGreeting ? `${greet(user.display_name)}\n\n想問你一個問題：` : '對了，想問你一個問題：';
  return `${opening}你是不是也瘦下來過，然後又胖回去了？
