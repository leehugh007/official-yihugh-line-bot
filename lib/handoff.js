...
async function notifyHandoff(userId, reason) {
  ...
  const reasonZh = {
    ...
    // 2026-04-26 Phase 4.6：學員故事 Flex 三按鈕（既有，v3.3 補中文化）
    q4_story_question: '學員故事 Flex「有問題想問」',
    q4_story_maybe: '學員故事 Flex「我再想想」', // 注意：q4_story_maybe 在 webhook 不 handoff，此 dict 僅備用
    // 2026-05-04 Phase 2A：v3.2 自動推進漏斗 4 段「我有問題想問」
    ...
    // 2026-05-05 Phase 2B Session 3：看過價格後傳自由文字 → 一律 handoff（一休 5/5 決策）
    // stage=6 + q5_msg{3,4}_sent_at NOT NULL + 自由文字（含貼圖照片）→ 不走 fallback 直接接人工
    q5_msg3_freetext: '訊息 3（課程+早鳥）後傳訊息',
    q5_msg4_freetext: '訊息 4（最後提醒）後傳訊息',
    // 2026-05-05 Phase 2B Session 3 補洞：stage=4 中性問題（≥4 字非意願詞非 negation）/ 非文字 → handoff
    // 接住「適合產後嗎」「ABC 是什麼」這類用戶有問題卻沒按按鈕的情境
  ...
  // v3.2 漏斗進度行（Ch.6.2 必修 M）— 命中 v3.2 reason 才插，舊 reason 不影響
  // question = 按「我有問題想問」按鈕；freetext = stage=6 看過價格後傳自由文字（一休 5/5 決策）
  const v32QuestionStages = {
    q5_msg1_question: '訊息 1（承接故事）',
    q5_msg2_question: '訊息 2（介紹 ABC）',
    q5_msg3_question: '訊息 3（課程+早鳥）',
    q5_msg4_question: '訊息 4（最後提醒）',
  };
  const v32FreetextStages = {
    q5_msg1_freetext: '訊息 1（承接故事）',
    q5_msg2_freetext: '訊息 2（介紹 ABC）',
    q5_msg3_freetext: '訊息 3（課程+早鳥）',
    q5_msg4_freetext: '訊息 4（最後提醒）',
  };
  const v32QuestionStage = v32QuestionStages[reason];
  const v32FreetextStage = v32FreetextStages[reason];
  ...
  if (v32QuestionStage) {
    lines.push(`v3.2 進度：${v32QuestionStage} 後問問題`);
  } else if (v32FreetextStage) {
    lines.push(`v3.2 進度：${v32FreetextStage} 後傳訊息`);
  }
  lines.push(`痛點：${pains.length ? pains.join('、') : '無'}`);
  lines.push('');
  lines.push(`→ 看用戶詳情：https://official-yihugh-line-bot.vercel.app/admin/user/${userId}`);
