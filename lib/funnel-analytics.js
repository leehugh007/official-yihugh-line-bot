...
async function fetchData({ rangeDays } = {}) {
  ...
  // 報名資料 — 只抓有 line_user_id 的（manual_offline source 可能 null）
...
export async function getFunnelStats({ rangeDays } = {}) {
  ...
  let total = 0;
  let inBTrack = 0;       // path_stage >= 1
  let finishedQ4 = 0;     // path_stage >= 4
  ...
  // 5. maybe_dist（按「我再想想」按鈕分佈）
  const maybeDist = { msg1: 0, msg2: 0, msg3: 0, msg4: 0 };

  // 6. intent_dist（AI Q5 意願分類）
  const intentDist = { continue: 0, decline: 0, ai_failed: 0, null: 0 };

  // 7. trigger_dist（被動 / 主動觸發）
  const triggerDist = { passive: 0, active: 0, null: 0 };

  users.forEach((u) => {
    total++;
    const stage = u.path_stage || 0;
    if (stage >= 1) inBTrack++;
    if (stage >= 4) finishedQ4++;
    ...
    if (u.q5_msg1_maybe_at) maybeDist.msg1++;
    if (u.q5_msg2_maybe_at) maybeDist.msg2++;
    if (u.q5_msg3_maybe_at) maybeDist.msg3++;
    if (u.q5_msg4_maybe_at) maybeDist.msg4++;

    if (u.q5_msg1_sent_at) {
      const intent = u.q5_intent;
      if (intent === 'continue') intentDist.continue++;
      else if (intent === 'decline') intentDist.decline++;
      else if (intent === 'ai_failed') intentDist.ai_failed++;
      else intentDist.null++;

      const trig = u.q5_followup_trigger_source;
      if (trig === 'passive') triggerDist.passive++;
      else if (trig === 'active') triggerDist.active++;
      else triggerDist.null++;
    }
  });

  return {
    range: { days: rangeDays || null },
    overall: { total, in_b_track: inBTrack, finished_q4: finishedQ4, ...overall },
    by_path: byPath,
    by_metabolism: byMetabolism,
    by_path_metabolism: byPathMetabolism,
    maybe_dist: maybeDist,
