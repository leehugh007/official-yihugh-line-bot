...
const BUTTON_TRACKING_STARTED_AT = '2026-05-22T08:41:23.000Z'; // 2026-05-22 16:41:23 +08:00
...
// 「獲得檢測報告」= 來自任一測驗工具（會收到該工具的報告 + 引導 CTA）
// 不在這 list 的（direct / seminar / legacy）= 沒做過任何測驗
const REPORT_SOURCES = ['quiz', 'protein', 'fatty_liver', 'tdee', 'blood_sugar', 'sugar'];
...
const USER_COLUMNS = [
  ...
  'ai_tags',
  'joined_at',
  'path_stage_updated_at',
  'last_interaction_at',
  'q5_click_count',
  'q5_clicked_at',
  'q5_apply_from_msg',
  'enrolled_at',
  // Q4 末尾 3 按鈕 + 中間層學員故事 Flex 3 按鈕（migration_023）
  'q4_continue_at', 'q4_maybe_at', 'q4_decline_at',
  'q4_story_interested_at', 'q4_story_question_at', 'q4_story_maybe_at',
  // msg1
...
function emptyFunnel() {
  return {
    got_report: 0,    // source in REPORT_SOURCES（拿到任一測驗報告 + 3 段訊息）
...
function accumulate(funnel, user, app) {
  if (REPORT_SOURCES.includes(user.source)) funnel.got_report++;
...
// 空的訊息反應物件
// - next: 按「往下走」(Q4=想聽聽 / Flex=想了解 ABC / msg1=想知道 / msg2=想看 12 週 / msg3-4=點 URI)
// - maybe: 按「我再想想」或「再考慮看看」
// - question: 按「我有問題想問」（Flex + msg1-4 有，Q4 沒有）
// - decline: 按「不適合」（只有 Q4 有）
function emptyMsgReaction() {
  return { sent: 0, next: 0, maybe: 0, question: 0, decline: 0 };
}

function isAtOrAfter(value, cutoff = BUTTON_TRACKING_STARTED_AT) {
...
export async function getFunnelStats({ rangeDays } = {}) {
  ...
  //    q4 / story 是 Phase 4.2 / 4.6 加的兩段，DB 欄位 migration_023 補（解開 Q4 → Q5 中間流失黑盒）
  //    no_response = sent - next - maybe - question - decline（前端算）
  const msgReactions = {
    q4: emptyMsgReaction(),
    story: emptyMsgReaction(),
  ...
  users.forEach((u) => {
    ...
    // Q4/story button tracking started on 2026-05-22 16:41 +08.
    // Use a tracked denominator only; older Q4 recipients did not have button fields yet.
    const q4TrackedAt = u.ai_tags?.q4_classified_at || u.path_stage_updated_at;
    const q4Tracked = (u.path_stage || 0) >= 4 && isAtOrAfter(q4TrackedAt);
    if (q4Tracked) msgReactions.q4.sent++;
    if (isAtOrAfter(u.q4_continue_at)) msgReactions.q4.next++;
    if (isAtOrAfter(u.q4_maybe_at)) msgReactions.q4.maybe++;
    if (isAtOrAfter(u.q4_decline_at)) msgReactions.q4.decline++;

    // Story Flex is sent only after q4_continue, so q4_continue_at is the tracked sent proxy.
    if (isAtOrAfter(u.q4_continue_at)) msgReactions.story.sent++;
    if (isAtOrAfter(u.q4_story_interested_at)) msgReactions.story.next++;
    if (isAtOrAfter(u.q4_story_maybe_at)) msgReactions.story.maybe++;
    if (isAtOrAfter(u.q4_story_question_at)) msgReactions.story.question++;
  ...
  return {
    ...
    tracking: {
      button_stats_started_at: BUTTON_TRACKING_STARTED_AT,
    },
  };
}
