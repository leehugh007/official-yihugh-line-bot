...
export async function getFunnelStats({ rangeDays } = {}) {
  ...
  users.forEach((u) => {
    ...
    // Q4 末尾 3 按鈕（sent = path_stage >= 4，因為到 stage=4 就收到 Q4 + 3 按鈕）
    if ((u.path_stage || 0) >= 4) msgReactions.q4.sent++;
    if (u.q4_continue_at) msgReactions.q4.next++;
    if (u.q4_maybe_at) msgReactions.q4.maybe++;
    if (u.q4_decline_at) msgReactions.q4.decline++;

    // 中間層學員故事 Flex 3 按鈕（sent = 按過 q4_continue，因為按完才推 Flex）
    if (u.q4_continue_at) msgReactions.story.sent++;
    if (u.q4_story_interested_at) msgReactions.story.next++;
    if (u.q4_story_maybe_at) msgReactions.story.maybe++;
    if (u.q4_story_question_at) msgReactions.story.question++;
