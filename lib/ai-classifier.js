...
/**
 * callGemini wrapper：自動重試偶發類錯誤（4/29 事故根治）
 *
 * 重試類型（Google 端 / 網路偶發）：
 *   - gemini_timeout / gemini_no_text / gemini_json_parse / gemini_api_5xx
 * 不重試（設定 / 內容問題）：
 *   - gemini_no_key / gemini_api_4xx
 *
 * Backoff 策略：
 *   - 503 後等 1.5s（Google 在說「太忙」，立刻重打多半還是 503）
 *   - timeout 不加 backoff（已等了 timeoutMs，自然就是 backoff）
 *
 * 預設 maxAttempts=3：3 次都失敗 caller 走 fallback（caller 已有現成 try-catch）
 *
 * @param {string} prompt
 * @param {{ model: string, timeoutMs: number }} cfg
 * @param {{ maxAttempts?: number }} [options]
 */
export async function withGeminiRetry(prompt, cfg, options = {}) {
  const maxAttempts = options.maxAttempts ?? 3;
  let lastErr;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await callGemini(prompt, cfg);
    } catch (err) {
      lastErr = err;
      ...
      const isRetriable =
        msg === 'gemini_timeout' ||
        msg === 'gemini_no_text' ||
        msg.startsWith('gemini_json_parse') ||
        /^gemini_api_5\d{2}/.test(msg);

      if (!isRetriable || attempt === maxAttempts) throw err;

      if (/^gemini_api_503/.test(msg)) {
        await new Promise((r) => setTimeout(r, 1500));
      }

      console.warn(
        `[withGeminiRetry] attempt ${attempt}/${maxAttempts} failed: ${msg.slice(0, 100)}, retrying`
      );
    }
  }
  throw lastErr;
