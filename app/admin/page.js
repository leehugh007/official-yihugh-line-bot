...
function AudienceRetargetingPrototype() {
  ...
  const [customArticleType, setCustomArticleType] = useState('student_story');
  ...
  const updateDraftButton = (index, field, value) => {
    setTemplateDraft((draft) => {
      ...
      if (field === 'actionType' && value === 'message' && !nextButton.messageText) {
    ...
    <div>
          ...
          {preset.id === 'custom' ? (
            <div style={styles.retargetingTuning}>
              ...
              <div style={styles.retargetingPrototypeNotice}>
                目前自訂受眾會跟這次自動再行銷規則一起儲存；後續若要管理多組受眾，會再擴充受眾庫、版本與快照。
              </div>
            </div>
          ) : (
        ...
        <section style={styles.retargetingPanel}>
          ...
          {selectedTemplate === 'custom' && (
            <div style={styles.retargetingTuning}>
              ...
              <div style={styles.retargetingPrototypeNotice}>
                目前自訂模板會跟這次自動再行銷規則一起儲存；後續若要管理多組模板，會再擴充模板庫、版本、啟用/停用與複製功能。
              </div>
            </div>
          )}
          ...
          {[0, 1, 2].map((index) => (
            <div key={index} style={styles.retargetingButtonEditor}>
              ...
              {(templateDraft.buttons[index]?.actionType || 'url') === 'message' ? (
                <>
                  <input
                    value={templateDraft.buttons[index]?.messageText || templateDraft.buttons[index]?.label || ''}
                    onChange={(e) => updateDraftButton(index, 'messageText', e.target.value)}
                    style={{ ...styles.input, marginBottom: 4 }}
                    placeholder="點擊後送出的文字，例如：我想問問題"
                  />
                  <textarea
          ...
          <div style={styles.retargetingFooter}>
            <div>
              <strong>模板編輯完成後請先儲存</strong>
              <span style={styles.retargetingMuted}>會連同目前受眾條件、發送流程與自動規則一起保存。</span>
            </div>
            <button
              type="button"
              style={styles.btnSecondary}
              disabled={adminAutoSaving || !templateDraft.body.trim()}
              onClick={() => handleSaveAdminAutoConfig(adminAutoEnabled)}
            >
              {adminAutoSaving ? '儲存中...' : '儲存模板與目前設定'}
            </button>
          </div>
          {templateSaveResult && (
            <div style={{
              ...
              {templateSaveResult.error || '模板與目前設定已儲存'}
            </div>
          )}
      ...
      <section style={{ ...styles.retargetingPanel, marginTop: 16 }}>
        <div style={styles.retargetingPanelHead}>
          ...
          <div>
            ...
            <p style={styles.retargetingMuted}>這段是自動觸發門檻與後續流程：第 1 區決定受眾情境，這裡決定系統何時判定未互動、何時發送、送出後怎麼接續。</p>
          </div>
        </div>
        ...
        <div style={styles.retargetingTimeline}>
          <div style={styles.retargetingTimelineItem}>
            ...
            <div style={styles.retargetingTimelineBody}>
              <strong>排程文章發出後，先等待再檢查互動</strong>
              <span style={styles.retargetingMuted}>這是自動流程實際使用的觸發門檻；第 1 區是受眾情境與排除條件，兩邊不會互相覆蓋。</span>
              <div style={styles.retargetingFlowGrid}>
                <div>
                  <label style={styles.fieldLabel}>自動流程：已收到至少幾篇排程文章</label>
                  <input
                    type="number"
                    min="1"
                    max="20"
                    value={adminAutoReceivedMin}
                    onChange={(e) => setAdminAutoReceivedMin(Number(e.target.value || 1))}
                    style={styles.input}
                  />
                </div>
                <div>
                  <label style={styles.fieldLabel}>自動流程：最近連續幾篇沒有點擊</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    value={adminAutoMissedSteps}
                    onChange={(e) => setAdminAutoMissedSteps(Number(e.target.value || 1))}
                    style={styles.input}
                  />
                </div>
                <div>
                  <label style={styles.fieldLabel}>每篇排程文章發出後，等待 {adminAutoWaitDays} 天再檢查最近互動</label>
                  <input
                    type="range"
                    min="0"
                    max="14"
      ...
      <section style={{ ...styles.retargetingPanel, marginTop: 16 }}>
        ...
        <div style={styles.retargetingRuleBox}>
          ...
          <span style={styles.retargetingRuleChip}>收到至少 {adminAutoReceivedMin} 篇</span>
        ...
        <div style={styles.retargetingAutomationPreview}>
          ...
          <div style={styles.retargetingFlowArrow}>→</div>
          <div style={styles.retargetingFlowNode}>已收到至少 {adminAutoReceivedMin} 篇</div>
          <div style={styles.retargetingFlowArrow}>→</div>
          <div style={styles.retargetingFlowNode}>最近連續 {adminAutoMissedSteps} 篇未點擊</div>
          <div style={styles.retargetingFlowArrow}>→</div>
          <div style={styles.retargetingFlowSplit}>
        ...
        <div style={styles.retargetingPrototypeNotice}>
          例：如果預排 10 篇，條件是收到至少 2 篇、最近連續 2 篇未點擊，當第 4 篇發出並等待完成後，就檢查第 3、4 篇；符合就觸發再行銷，不會等到第 10 篇。
        </div>
