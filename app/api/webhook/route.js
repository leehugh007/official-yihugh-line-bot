...
async function handleConversationPath(event, userId, text, state) {
  ...
  if (stage === 2) {
    ...
    if (!choice) {
      ...
      // 2b. 其他自由文字（2026-04-24 定調：盡可能讓用戶選固定選項）
      //
      // 原本 PR #46 設計「≥5 字自由文字 → handoff」，但一休發現太容易誤觸
      //（「產後+外食」這種很多人的複選狀況也會被當「特殊」→ 過早進人工）。
      // 改為：一律引導回 A/B/C/D 選項。用戶真的特殊狀況（A-D 都不適用）
      // 等她進 Q3→Q4→stage=4 bridging 或 Q5 wire 後再接住。
      await replyMessage(event.replyToken, [
